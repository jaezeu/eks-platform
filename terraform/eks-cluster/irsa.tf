...
module "external_dns_role" {
  ...
  create_role                    = true
  role_name                      = "${local.name_prefix}-cluster-externaldns-role"
...
module "ebs_csi_driver_role" {
  ...
  create_role                    = true
  role_name                      = "${local.name_prefix}-cluster-ebs-csidriver-role"
  provider_url                   = module.eks.oidc_provider
  oidc_fully_qualified_audiences = ["sts.amazonaws.com"]
...
module "loki_s3_role" {
  ...
  create_role                    = true
  role_name                      = "${local.name_prefix}-cluster-loki-s3-role"
  provider_url                   = module.eks.oidc_provider
  oidc_fully_qualified_audiences = ["sts.amazonaws.com"]
